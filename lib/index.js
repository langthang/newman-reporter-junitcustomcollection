...
/**
 * Extend a name with prefix, separate by separator
 *
 * @param {?String} prefix The prefix to extend the name
 * @param {?String} name The original name
 * @param {?String} [separator=SEP] The separator symbol to join path name entries with
 * @returns {String} The full name extended
 * @private
 */
function extendName(prefix, name, separator)  {
    if (_.isEmpty(prefix)) {return name}
 ...
 * @returns {*}
 */
JunitCustomCollectionReporter = function (newman, reporterOptions) {
    newman.on('beforeDone', function () {
        var report = _.get(newman, 'summary.run.executions'),
            collection = _.get(newman, 'summary.collection'),
            customCollectionName = reporterOptions.classname || collection.name,
            cache,
            root,
        ...
        classname = _.upperFirst(_.camelCase(customCollectionName).replace(/\W/g, ''));

        root = xml.create('testsuites', { version: '1.0', encoding: 'UTF-8' });
        root.att('name', customCollectionName);
        ...
        _.forEach(cache, function (executions, itemId) {
            ...
            _.forOwn(tests, function (failures, name) {
                var testcase = suite.ele('testcase'),
                    ...
                    testcase.att('name', extendName(
                        _.get(testcase.up(), 'attributes.name.value', classname), 
                        name));
                testcase.att('time', executionTime.toFixed(3));

                // Set the same classname for all the tests
                testcase.att('classname', classname);
                ...
                if (failures && failures.length) {
                    ...
                    failure.dat('Collection name: ' + customCollectionName + '.');
...
module.exports = JunitCustomCollectionReporter;
